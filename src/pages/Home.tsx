...
import { fetchPopularMovies, fetchUpcomingMovies, fetchActionMovies, fetchGenres } from '../api/tmdb';
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchActionMovies().then(setAction).catch(console.error);
    fetchGenres().then(setGenres).catch(console.error);
  }, []);

  if (popular.length === 0 || upcoming.length === 0 || Action.length === 0 || genres.length === 0) return <div>Chargement...</div>;
  ...
  return (
    <Container fluid className='background'>
      <Row>
        <div
          style={{
            backgroundImage: `linear-gradient(to right, #2a008a, rgba(0, 0, 0, 0.2)), url(https://image.tmdb.org/t/p/original${mainPopular.backdrop_path})`,
      ...
      <Row>
        <div
          style={{
            backgroundImage: `linear-gradient(to left, #2a008a, rgba(0, 0, 0, 0.2)), url(https://image.tmdb.org/t/p/original${mainUpcoming.backdrop_path})`,
            backgroundSize: 'cover',
      ...
      <Row className='pt-5'>
        <div
          style={{
            backgroundImage: `linear-gradient(to right, #2a008a, rgba(0, 0, 0, 0.2)), url(https://image.tmdb.org/t/p/original${mainAction.backdrop_path})`,
            backgroundSize: 'cover',
            backgroundPosition: 'top',
            color: 'white',
            padding: '150px 30px',
          ...
          <div className="scroll-track">
            {[...genres, ...genres].map((genre, idx) => (
              <div className="scroll-categorie" key={idx}>
                <span style={{ color: '#2a008a', fontWeight: 'bold' }}>{genre.name}</span>
              </div>
            ))}
          </div>
        </div>
        <Link to={`/categorie/:genreId`}>
          <button className='second-costum-button'>Voir plus</button>
        </Link>
      </Row>
      <Row className='footer'>
        <Col className='reseau'>
          <h3>Suivez-nous !</h3>
